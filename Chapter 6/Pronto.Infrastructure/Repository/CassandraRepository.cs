...
{
    public class CassandraRepository : IProntoRepository<CassandraChunk, float[]>
    {
        /// <summary>
        /// DB initialization to load all embedding for every chunk
        /// </summary>
        /// <returns></returns>
        public int AddInitialEmbeddings(IList<(CassandraChunk chunk, float[] embeddedChunk)> embeddings)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public int AddEmbedding(ProntoContext _context, (CassandraChunk chunk, float[] embeddedChunk) embedding)
        {
            _context.CassandraChunks.Add(embedding.chunk);
            _context.SaveChanges();
        ...
        /// </summary>
        /// <returns></returns>
        public int AddEmbedding((CassandraChunk chunk, float[] embeddedChunk) embedding)
        {
            using var _context = new ProntoContext();
            _context.CassandraChunks.Add(embedding.chunk);
            _context.SaveChanges();
